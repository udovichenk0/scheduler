...
test('should send an email', async () => {
  ...
  expect(transporter.sendEmail)
    .toBeCalledWith({to: email, code, subject: 'Verify your email'})
})
...
test("should verify otp", async () => {
  ...
  jest.spyOn(otpRepository, 'findByUserEmail').mockResolvedValue({code, user})
  jest.spyOn(otpRepository, 'deleteByUserId').mockResolvedValue({id: '123', code, user_id: user.id})
...
test('should return Err object if arguments are invalid or missing', async () => {
  ...
  const invalidEmail = await otpService.verifyOTP({code, email: "invalidemail"})
