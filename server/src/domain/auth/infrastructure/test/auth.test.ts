...
beforeEach(() => {
  ...
  tokenService = new TokenService(refreshService, userService, jwtService);
  emailConfirmationService = new OTPService(
    userService,
    otpRepository,
    new EmailTransporter()
  );
...
test("should create user", async () => {
  ...
  jest
    ...
    .mockResolvedValue(Errors.UserNotFound(user.email));
...
test("should return Err object if creation of user failed", async () => {
  jest
    .spyOn(userService, "findByEmail")
    .mockResolvedValue(Errors.UserNotFound("denzeldenisq@gmail.com"));
  jest
    .spyOn(userService, "createOne")
    .mockResolvedValue(Errors.InternalServerError());
  jest.spyOn(emailConfirmationService, "create").mockResolvedValue({
    id: "123",
    code: "",
    user_id: "asdf"
...
test("should return Err object if creation of otp failed", async () => {
  ...
  jest.spyOn(emailConfirmationService, "sendEmail");
  jest
    .spyOn(emailConfirmationService, "create")
    .mockReturnValue(Errors.InternalServerError());
...
test("should not verify user if the hash is not valid", async () => {
  ...
  expect(error).toStrictEqual(Errors.GeneralInvalid("Password", "123"));
});
...
test("should verify email", async () => {
  ...
  jest
    .spyOn(emailConfirmationService, "verifyOTP")
    .mockResolvedValue(otpResult);
