import { attach, createEvent, createStore, sample, createEffect } from "effector";
...
export const $isWorkTime = createStore(true)
.on(toggleTimerState, (isWorkTime) => !isWorkTime)
...
sample({
  ...
  fn: ({currentStaticTime, progress}) => progress - (DEFAULT_PROGRESS_BAR / currentStaticTime),
  target: $progress
})

sample({
  clock: timeSelected,
  target: [$passingTime,$currentStaticTime, $selectedTime, $progress.reinit, stopTimerTriggered]
})

sample({
  clock: resetTimerTriggered,
  target: [$passingTime, $progress.reinit,$isWorkTime.reinit, $kvStages.reinit,$activeStageId.reinit, stopTimerTriggered]
})
sample({
  clock: resetTimerTriggered,
  source: $selectedTime,
...
sample({
  ...
  fn: ({activeStageId, kv}) => ({...kv, [activeStageId]: {fulfilled: true}}),
  target: $kvStages
})
sample({
  clock: timePassed,
