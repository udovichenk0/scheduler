...
export const Header = ({ icon, title }: { icon: IconName; title: string }) => {
  ...
  return (
    <div className="mb-5 px-4 text-primary">
      <div className="py-2 h-[40px] flex items-center justify-end">
      { isPomodoroRunning 
      ? <PomodoroProgressBar onClick={togglePomodoroModal}/> 
      : <Button intent={"primary"} size={"xs"} onClick={togglePomodoroModal}>
          <Icon className="text-[24px] text-cIconDefault" name="common/timer"/>
        </Button>
    }
    </div>
      <Pomodoro
        ...
        leftSlot={
          <>
            <Button intent={"primary"} size={"xs"} onClick={toggleSettingsModal}>
              <Icon className="text-[24px] text-cIconDefault" name="common/settings"/>
            </Button>
