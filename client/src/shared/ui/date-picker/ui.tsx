...
export function DatePicker({
  ...
  const endObserver = new IntersectionObserver((elem) => {
  ...
  const startObserver = new IntersectionObserver((elem) => {
    if(elem[0].isIntersecting && top > 0){
      const newCalendars = generateCalendar(dayjs().add(weeksCount - 25, 'month'))
      const calendarsLength = newCalendars.length
      if(weeksCount > 20){
              ...
              {calendars.map(({rows, date}, id) => {
                return (
                  <Fragment key={id}>
                    ...
                    <Calendar rows={rows} sectionDate={date} onDateChange={onDateChange} currentDate={currentDate}/>
