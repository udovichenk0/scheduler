...
export const Button = ({ intent, size, children, className, ...props }: ButtonOrLinkProps) => {
  if(props.as == 'link'){
    return (
      <Link {...props} to={props.to} className={clsx(className, buttonCva({size, intent}))}>{children}</Link>
    )
  }
  return (
