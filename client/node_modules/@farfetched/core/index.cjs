...
function abortable(config) {
  ...
  const runCallFx = effector.createEffect(_async$7(function (def) {
    return def.promise;
  }));
  const $calls = effector.createStore([], {
    serialize: 'ignore'
  });
  const callsApi = effector.createApi($calls, {
    add(calls, def) {
      return [...calls, def];
    },
  ...
  if (abort === null || abort === void 0 ? void 0 : abort.signal) {
    const abortTrigger = effector.sample({
      clock: abort.signal
    });
  ...
  const handler = _async$7(function (...args) {
    return effect(...args);
  });

  const runnerFx = effector.createEffect({
    name: (_a = config.name) !== null && _a !== void 0 ? _a : 'runnerFx',
    sid: ((_b = config.name) !== null && _b !== void 0 ? _b : 'runnerFx') + getId(),
    handler: _async$7(function (p) {
      const {
        runAborters,
        onAbort
      } = createAborter();
      ...
      try {
        boundApiRemove = effector.scopeBind(callsApi.remove);
      } catch (e) {
...
function normalizeSourced({
  ...
  let $target = effector.createStore(null, {
    serialize: 'ignore'
  });

  if (clock) {
    if (field === undefined) ; else if (effector.is.store(field)) {
  ...
  if (source) {
    ...
    if (field === undefined) ; else if (effector.is.store(field)) {
      ...
      const callbackField = field;
      $target = effector.combine($source, callbackField.source, callbackField.fn);
    } else if (typeof field === 'function') {
      const callbackField = field;
      $target = $source.map(callbackField);
    } else {
      const valueField = field;
      $target = effector.createStore(valueField, {
        serialize: 'ignore'
      });
    }
  }
...
function createSourcedReader(field) {
  ...
  if (field === undefined) {
    readFx = effector.createEffect(function (_params) {
      return _await$6(null);
    });
  } else if (effector.is.store(field)) {
    const $storeField = field;
    readFx = effector.attach({
      source: $storeField,
      effect: function (source, _params) {
        return _await$6(source);
      }
    });
    ...
    readFx = effector.attach({
      source: callbackField.source,
      effect: _async$6(function (source, params) {
        return callbackField.fn(params, source);
      })
    });
  } else if (typeof field === 'function') {
    const callbackField = field;
    readFx = effector.createEffect(_async$6(function (params) {
      return callbackField(params);
    }));
  } else {
    const valueField = field;
    readFx = effector.createEffect(function (_params) {
      return _await$6(valueField);
    });
  }
...
function normalizeStaticOrReactive(v) {
  if (!v) {
    return effector.createStore(null, {
      serialize: 'ignore'
    });
  }

  if (effector.is.store(v)) {
    return v;
  }

  return effector.createStore(v, {
    serialize: 'ignore'
  });
} // -- Extract source
...
function combineSourced(config, mapper) {
  ...
  const $megaSource = effector.combine(megaStore);
  return {
    source: $megaSource,
    fn: (data, source) => {
      const result = {};
...
function delay({
  clock,
  timeout,
  target = effector.createEvent()
}) {
  const timerFx = effector.createEffect(({
    payload,
    milliseconds
  }) => new Promise(resolve => {
...
function every(configOrStores, predicateOrNone) {
  ...
  if (isFunction(predicate)) {
    checker = predicate;
  } else if (effector.is.store(predicate)) {
    checker = predicate.map(value => required => value === required);
  } else {
    checker = value => value === predicate;
  }

  const $values = effector.combine(stores); // Combine pass simple values as is

  const $checker = checker;
  return effector.combine($checker, $values, (checker, values) => values.every(checker));
}
...
function postpone({
  ...
  const target = effector.createEvent();
  const $fired = effector.createStore(false, {
    serialize: 'ignore'
...
const readNowFx = effector.createEffect(() => Date.now());
function time({
  clock
}) {
  const $time = effector.createStore(Date.now());
  effector.sample({
    clock: clock,
    fn: () => {// nothing
    },
...
function syncBatch(clock) {
  const saveTimeoutId = effector.createEvent();
  const $timeoutId = effector.createStore(null, {
    serialize: 'ignore'
  }).on(saveTimeoutId, (_, id) => id);
  const saveReject = effector.createEvent();
  const $rejecter = effector.createStore(null, {
    serialize: 'ignore'
  }).on(saveReject, (_, rj) => rj);
  const tick = effector.createEvent();
  const timerFx = effector.attach({
    source: {
      timeoutId: $timeoutId,
  ...
  const $payload = effector.createStore([], {
    serialize: 'ignore'
  }).on(clock, (_, payload) => [payload]);
  const $canTick = effector.createStore(true, {
    serialize: 'ignore'
  });
  const triggerTick = effector.createEvent();
...
function createContractApplier(contract) {
  const applyContractFx = effector.createEffect({
    handler: ({
      result: data
    }) => {
...
function createRemoteOperation({
  ...
  const revalidate = effector.createEvent();
  ...
  const executeFx = effector.createEffect({
    handler: () => {
  ...
  const remoteDataSoruce = {
    name: 'remote_source',
    get: effector.createEffect(_async$5(function ({
      params
    }) {
      return _await$5(executeFx(params), function (result) {
        return {
          result,
          stale: false
        };
  ...
  const start = effector.createEvent(); // Signal-events

  const finished = {
    success: effector.createEvent(),
    failure: effector.createEvent(),
    skip: effector.createEvent(),
    finally: effector.createEvent()
  }; // -- Main stores --

  const $status = effector.createStore('initial', {
    sid: `ff.${name}.$status`,
    name: `${name}.$status`,
    serialize
  });
  const $enabled = normalizeStaticOrReactive(enabled !== null && enabled !== void 0 ? enabled : true).map(Boolean);
  const $latestParams = effector.createStore(null, {
    serialize: 'ignore'
  }); // -- Derived stores --
  ...
  } = effector.split(effector.sample({
...
function createDataSourceHandlers(dataSources) {
  const retrieveDataFx = effector.createEffect({
    handler: _async$5(function ({
      params,
      skipStale
    }) {
      let _exit = false;
      return _continue(_forOf$1(dataSources, function (dataSource) {
        return _await$5(dataSource.get({
          params
        }), function (fromSource) {
  ...
  const notifyAboutNewValidDataFx = effector.createEffect({
    handler: _async$5(function ({
      params,
      result
    }) {
      return _awaitIgnored$2(Promise.all(dataSources.map(get('set')).filter(Boolean).map(set => set({
        params,
        result
      }))));
    })
  });
  const notifyAboutDataInvalidationFx = effector.createEffect({
    handler: _async$5(function ({
      params
    }) {
      return _awaitIgnored$2(Promise.all(dataSources.map(get('unset')).filter(Boolean).map(unset => unset({
        params
      }))));
    })
  });
...
function createHeadlessQuery(config) {
  ...
  const reset = effector.createEvent(); // -- Main stores --

  const $data = effector.createStore(initialData, {
  ...
  const $stale = effector.createStore(true, {
  ...
  const attachProtocol = ({
    ...
    attachedQuery.__.lowLevelAPI.dataSourceRetrieverFx.use(effector.attach({
      source,
...
function resolveExecuteEffect(config) {
  ...
  if (effector.is.effect(anyConfig.effect)) {
    return anyConfig.effect;
  } else if (typeof anyConfig.handler === 'function') {
    return effector.createEffect(anyConfig.handler);
  }
...
function connectQuery(args) {
  ...
  const anyParentStarted = effector.merge(parents.map(query => query.start));
  const anyParentSuccessfullyFinished = effector.merge(parents.map(query => query.finished.success));
  const $allParentsHaveData = every({
    stores: parents.map(query => query.$data),
    predicate: data => data !== null
  });
  const $allParentDataDictionary = singleParentMode ? source.$data : effector.combine(mapValues(source, query => query.$data));
  const $allParentParamsDictionary = effector.createStore(null, {
    serialize: 'ignore'
  });
...
const fetchFx = effector.createEffect({
  sid: 'ff.fetchFx',
...
const requestFx = effector.createEffect({
  handler: _async$4(function (request) {
    var _a;

    return _await$4(fetchFx(request).catch(cause => {
      var _a;
      ...
      return _invoke$1(function () {
        if (!response.ok) {
          const _response$statusText = response.statusText,
                _response$status = response.status;
          return _await$4(response.text().catch(() => null), function (_response$text$catch) {
            throw httpError({
              status: _response$status,
              statusText: _response$statusText,
              response: (_a = _response$text$catch) !== null && _a !== void 0 ? _a : null
            });
          });
        }
      }, function (_result) {
...
function createApiRequest(config) {
  ...
  const prepareFx = effector.createEffect(config.response.extract);
  const $haveToBeAborted = effector.createStore(false, {
    serialize: 'ignore'
  });
  const apiRequestFx = effector.createEffect(_async$3(function ({
    url,
    method,
    query,
    headers,
    ...
    return _await$3(requestFx(request).catch(cause => {
      ...
      return _await$3(prepareFx(response).catch(_async$3(function (cause) {
        var _a;

        return _await$3(clonedResponse.text(), function (_clonedResponse$text) {
          throw preparationError({
            response: _clonedResponse$text,
            reason: (_a = cause === null || cause === void 0 ? void 0 : cause.message) !== null && _a !== void 0 ? _a : null
          });
        });
      })), function (prepared) {
  ...
  const boundApiRequestFx = effector.attach({
    source: {
  ...
  const abortSignal = effector.createEvent();
...
const isEmptyResponse = _async$2(function (response) {
  if (!response.body) {
    return true;
  }
  ...
  return _await$2(clonnedResponse.text(), function (bodyAsText) {
    return bodyAsText.length === 0 ? true : false;
  });
});
...
function createJsonApiRequest(config) {
  ...
  const $headers = effector.combine({
    method: normalizeStaticOrReactive(config.request.method),
    headers: normalizeStaticOrReactive(config.request.headers)
  }, ({
    method,
  ...
  const jsonApiCallFx = createApiRequest(_extends({}, config, {
    ...
    response: {
      extract: function (response) {
        return _await$2(isEmptyResponse(response), function (emptyContent) {
          return emptyContent ? null : response.json();
        });
      },
...
function createJsonQuery(config) {
  ...
  const internalStart = effector.createEvent();
  const headlessQuery = createHeadlessQuery({
  ...
  headlessQuery.__.executeFx.use(effector.attach({
    source: {
...
function createHeadlessMutation(config) {
  ...
  const attachProtocol = ({
    ...
    attachedMutation.__.lowLevelAPI.dataSourceRetrieverFx.use(effector.attach({
      source,
      mapParams: (_ref, sourceValue) => {
        let {
          params
...
function createJsonMutation(config) {
  ...
  headlessMutation.__.executeFx.use(effector.attach({
    source: {
      url: normalizeSourced({
        field: config.request.url,
        clock: internalStart
...
function retry(operation, {
  ...
  const $attempt = effector.createStore(1, {
    serialize: 'ignore'
  });
  const $meta = effector.combine({
    attempt: $attempt
  });
  const newAttempt = effector.createEvent();
  const {
    planNextAttempt,
    __: retriesAreOver
  } = effector.split(effector.sample({
...
function update(query, {
  ...
  const fillQueryData = effector.createEvent();
  const fillQueryError = effector.createEvent();
  effector.split({
  ...
  const {
    shouldRefetch,
    __: shouldNotRefetch
  } = effector.split(effector.merge([fillQueryData, fillQueryError]).map(({
    refetch
  }) => refetch), {
...
function attachOperation(operation, config) {
  ...
  return (_a = operation.__.experimentalAPI) === null || _a === void 0 ? void 0 : _a.attach({
    source: source !== null && source !== void 0 ? source : effector.createStore(null, {
      serialize: 'ignore'
    }),
...
function createCacheAdapter(adapter) {
  const $instance = effector.createStore(adapter, {
    serialize: 'ignore',
...
function inMemoryCache(config) {
  ...
  let storage = {};
  const saveValue = effector.createEvent();
  const removeValue = effector.createEvent();
  const itemExpired = effector.createEvent();
  const itemEvicted = effector.createEvent();
  const purge = effector.createEvent();
  purge.watch(() => {
    storage = {};
  });
  const $now = time({
  ...
  const maxEntriesApplied = effector.sample({
    clock: saveValue,
    source: {
      now: $now
    },
  ...
  if (maxAge) {
    ...
    saveValue.watch(payload => {
      const boundItemExpired = effector.scopeBind(itemExpired, {
        safe: true
      });
  ...
  const adapter = {
    ...
    set: effector.createEffect(saveValue),
    unset: effector.createEffect(removeValue),
    purge
  };
  attachObservability({
    adapter,
...
function cache(query, rawParams) {
  ...
  const readAllSourcedFx = effector.createEffect(_async$1(function (params) {
    return Promise.all(sourcedReaders.map(readerFx => readerFx(params)));
  }));
  const unsetFx = effector.createEffect(_async$1(function ({
    instance,
    params
  }) {
    return _await$1(readAllSourcedFx(params), function (sources) {
      const key = createKey({
        sid: id,
      ...
      return _awaitIgnored$1(instance.unset({
        key
      }));
    });
  }));
  const setFx = effector.createEffect(_async$1(function ({
    instance,
    params,
    result
  }) {
    return _await$1(readAllSourcedFx(params), function (sources) {
      ...
      return _awaitIgnored$1(instance.set({
        key,
        value: result
      }));
    });
  }));
  const getFx = effector.createEffect(_async$1(function ({
    params,
    instance
  }) {
    return _await$1(readAllSourcedFx(params), function (sources) {
      ...
      return key ? _await$1(instance.get({
        key
      }), function (result) {
        if (!result) {
          return null;
...
function externalCache(config) {
  const purge = effector.createEvent();
  const purgeFx = effector.createEffect(config.purge);
  effector.sample({
    clock: purge,
    target: purgeFx
  });
  const adapter = {
    get: effector.createEffect(config.get),
    set: effector.createEffect(config.set),
    unset: effector.createEffect(config.unset),
    purge
  };
  attachObservability({
    adapter,
...
function browserStorageCache(config) {
  ...
  function storageCache() {
    const getSavedItemFx = effector.createEffect(_async(function (key) {
      return _await(getItemFx(key), function (item) {
        if (!item) return null;
    ...
    const setSavedItemFx = effector.createEffect(_async(function ({
      key,
      value
    }) {
      ...
      return _awaitIgnored(setItemFx({
        key,
        value: item
      }));
    }));
    const removeSavedItemFx = effector.createEffect(_async(function (key) {
      metaStorage.removeKey({
        key
      });
      return _awaitIgnored(removeItemFx(key));
    }));
    const itemExpired = effector.createEvent();
    const itemEvicted = effector.createEvent();
    const purge = effector.createEvent();
    const purgeFx = effector.createEffect(_async(function (keys) {
      return Promise.all(keys.map(removeSavedItemFx));
    }));
    effector.sample({
      clock: purge,
    ...
    const adapter = {
      get: effector.createEffect(_async(function ({
        key
      }) {
        return _await(getSavedItemFx(key), function (saved) {
          let _exit = false;
          return saved ? _invoke(function () {
            if (maxAge) {
              ...
              return function () {
                if (Date.now() >= expiredAt) {
                  ...
                  return _await(removeSavedItemFx(key), function () {
                    const _temp = null;
                    _exit = true;
                    return _temp;
                  });
      ...
      set: effector.createEffect(function ({
        key,
        value
      }) {
        var _a, _b, _c;

        return _call(getMetaFx, function (meta) {
          ...
          return _invoke(function () {
            if (maxEntries && keysAmount >= maxEntries) {
              ...
              return _continueIgnored(_forOf(forDelete !== null && forDelete !== void 0 ? forDelete : [], function (key) {
                itemEvicted({
                  key
                });
                return _awaitIgnored(removeSavedItemFx(key));
              }));
            }
          }, function () {
            return _awaitIgnored(setSavedItemFx({
              key,
              value
            }));
          });
      ...
      unset: effector.createEffect(_async(function ({
        key
      }) {
        return _awaitIgnored(removeSavedItemFx(key));
      })),
  ...
  const $meta = effector.createStore(null, {
    serialize: 'ignore'
  });
  const getMetaFx = effector.createEffect(_async(function () {
    return _await(getItemFx(META_KEY), function (meta) {
      if (!meta) return null;
  ...
  const setMetaFx = effector.createEffect(meta => setItemFx({
    key: META_KEY,
    value: JSON.stringify(meta)
  }));
  const addKey = effector.createEvent();
  const removeKey = effector.createEvent();
  const metaStorage = {
    $meta,
    addKey,
    removeKey
  ...
  const setItemFx = effector.createEffect(params => {
    storage().setItem(params.key, params.value);
  });
  const getItemFx = effector.createEffect(key => storage().getItem(key));
  const removeItemFx = effector.createEffect(key => storage().removeItem(key)); // public
...
function voidCache() {
  return createCacheAdapter({
    get: effector.createEffect(() => null),
    set: effector.createEffect(() => {// pass
    }),
    purge: effector.createEvent(),
    unset: effector.createEffect()
  });
}

function declareParams() {
  return effector.createEvent();
}
...
function keepFresh(query, config) {
  ...
  const triggers = [];
  const [triggerEvents, protocolCompatibleObjects] = divide((_a = config.triggers) !== null && _a !== void 0 ? _a : [], effector.is.event);
  ...
  if (protocolCompatibleObjects.length > 0) {
    ...
    const $alreadySetup = effector.createStore(false, {
      serialize: 'ignore'
    });
    const {
      setup,
      teardown
    } = effector.createApi($alreadySetup, {
      setup: () => true,
      teardown: () => false
    });
    effector.sample({
      clock: [query.finished.success, effector.sample({
  ...
  if (config.automatically) {
    ...
    const $previousSources = effector.createStore([], {
      serialize: 'ignore'
    }); // @ts-expect-error I have no idea

    effector.sample({
      clock: finalyParams,
      source: effector.combine(query.__.lowLevelAPI.sourced.map(sourced => normalizeSourced({
        field: sourced,
        clock: finalyParams
      }))),
      filter: query.$enabled,
    ...
    const sourcesUpdated = effector.sample({
      clock: query.__.lowLevelAPI.sourced.map(extractSource).filter(effector.is.store),
      source: query.__.$latestParams,
      filter: not(query.$idle),
      fn: params => params
    });
    const $nextSources = effector.combine(query.__.lowLevelAPI.sourced.map(sourced => normalizeSourced({
      field: sourced,
      clock: sourcesUpdated
    })));
    triggers.push(effector.sample({
  ...
  const forceFresh = effector.sample({
    clock: triggers,
    filter: query.$enabled
  });
  effector.sample({
...
exports.attachOperation = attachOperation;
exports.cache = cache;
exports.combineSourced = combineSourced;
exports.connectQuery = connectQuery;
exports.createCacheAdapter = createCacheAdapter;
exports.createHeadlessMutation = createHeadlessMutation;
exports.createHeadlessQuery = createHeadlessQuery;
exports.createJsonMutation = createJsonMutation;
exports.createJsonQuery = createJsonQuery;
exports.createMutation = createMutation;
exports.createQuery = createQuery;
exports.declareParams = declareParams;
exports.exponentialDelay = exponentialDelay;
exports.externalCache = externalCache;
exports.fetchFx = fetchFx;
exports.httpError = httpError;
exports.inMemoryCache = inMemoryCache;
exports.invalidDataError = invalidDataError;
exports.isAbortError = isAbortError;
exports.isHttpError = isHttpError;
exports.isHttpErrorCode = isHttpErrorCode;
exports.isInvalidDataError = isInvalidDataError;
exports.isNetworkError = isNetworkError;
exports.isPreparationError = isPreparationError;
exports.isTimeoutError = isTimeoutError;
exports.keepFresh = keepFresh;
exports.linearDelay = linearDelay;
exports.localStorageCache = localStorageCache;
exports.networkError = networkError;
exports.normalizeSourced = normalizeSourced;
exports.preparationError = preparationError;
exports.retry = retry;
exports.sessionStorageCache = sessionStorageCache;
exports.timeoutError = timeoutError;
exports.unknownContract = unknownContract;
exports.update = update;
exports.voidCache = voidCache;
