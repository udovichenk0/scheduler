...
var RouteLinkView = function RouteLinkView(props, ref) {
  ...
  var isOpened = scope.useStore(routeObj.route.$isOpened);
  var navigate = scope.useEvent(to.navigate);
...
function useIsOpened(route) {
  return Array.isArray(route) ? route.map(function (route) {
    return scope.useStore(route.$isOpened);
  }).some(Boolean) : scope.useStore(route.$isOpened);
}
